...
for j in colnec:
    ...
    if check(j,coldf) == False: # Si el df no tiene la información del parámetro "MW", lo informa y solicita que ingrese esa información.
        ...
        #break
    if conta == 4:
        print("Los datos están completos, a continuación se analizará la información y se generará el gráfico.")       
...
# plt.style.use("ggplot")# seleccionamos el estilo de grafico para visualizar la información.
# fig, ax = plt.subplots(figsize = (15,8))# creamos el objeto "fig" configuramos el tamaño de los ejes del grafico.
# ax.plot(df['SPP'], label = "SPP", lw = 2, color = "#EC4D37", path_effects = [pa.SimpleLineShadow(), pa.Normal()])# indicamos que serie o columna vamos a graficar y algunas personalizaciones que queremos que tenga la serie.
# ax.plot(df['SPPT'], label = "SPPT", lw = 2, color = "#1D1818", linestyle = "-.", path_effects = [pa.SimpleLineShadow(), pa.Normal()])# indicamos que serie o columna vamos a graficar y algunas personalizaciones que queremos que tenga la serie.
# plt.show()
