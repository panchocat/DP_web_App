...
def pipe_profile():
    with st.form('pipe_profile', clear_on_submit=True):
        ...
        Name = st.text_input("Nombre del componente (ej. DP(Drill pipe) or BHA(Bottom hole asembly)) :red[*]: ",placeholder=None)
        Long = st.number_input("Longitud del componente (ft) :red[*]: ",value=None,placeholder="Type a number...")
        O_D = st.number_input("Diametro Externo del componente (in) :red[*]: ",value=None,placeholder="Type a number...")
        I_D = st.number_input("Diametro Interno del componente (in) :red[*]: ",value=None,placeholder="Type a number...")
        tfa = st.number_input("TFA de la broca (:warning: Ingrese esta información cuando agregue el último componente de la sarta) :red[*]: ",value=None,placeholder="Type a number...")
